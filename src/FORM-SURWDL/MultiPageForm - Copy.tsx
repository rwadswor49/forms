...
export default function MultiPageForm({ goHome, formTitle }: Props) {
  const totalPages = 7
  ...
  // --- Refs for validation ---
  const individualRef = useRef<{ submit: () => Promise<PolicyholderFormValues | null> }>(null)
  const trustRef = useRef<{ submit: () => Promise<TrustFormValues | null> }>(null)
  const companyRef = useRef<{ submit: () => Promise<CompanyFormValues | null> }>(null)
  const withdrawalRef = useRef<{ submit: () => Promise<SurrenderWithdrawalFormValues | null> }>(null)
  const bankRef = useRef<{ submit: () => Promise<PaymentInstructionsFormValues | null> }>(null)
  const assetRef = useRef<{ submit: () => Promise<AssetFormValues | null> }>(null)
  const sampleRef = useRef<{ submit: () => Promise<SampleFormValues | null> }>(null)
...
const goNext = async () => {
  setFormErrors([])
  setImportantNotesError("") // reset any previous error
...
// Always save the user's inputs, even if validation fails
if (data) {
  setWithdrawalData(data)
} else {
  const fallback = withdrawalRef.current?.getValues?.()
  if (fallback) setWithdrawalData(fallback)
}

valid = !!data


  // Always clear previous Important Notes error first
  setImportantNotesError("")

  // --- Important Notes Validation for surrender ---
  if (
    valid &&
...
const goPrev = () => {
  setImportantNotesError("")   // clear any previous error
  setFormErrors([])            // optional: clear other form errors as well
